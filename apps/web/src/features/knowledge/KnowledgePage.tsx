...
export function KnowledgePage({ workspace }: { workspace: Workspace }) {
  const client = useQueryClient(); const admin = workspace.role !== 'member'; const [tab, setTab] = useState<'milestones' | 'artifacts' | 'context'>('milestones'); const [selectedId, setSelectedId] = useState(''); const [query, setQuery] = useState(''); const [compose, setCompose] = useState(false);
  ...
  return <Page heading="Knowledge" subheading="Milestones, revisioned artifacts, canonical Git sources, and understandable task context packs." action={tab !== 'context' ? <button className="button primary" onClick={() => setCompose(true)}>New {tab === 'milestones' ? 'milestone' : 'artifact'}</button> : undefined}>
    ...
    {compose && (tab === 'milestones' ? <MilestoneComposer workspace={workspace} projects={projects.data?.items ?? []} close={() => setCompose(false)} done={() => { setCompose(false); client.invalidateQueries({ queryKey: ['milestones', workspace.id] }); }} /> : <ArtifactComposer workspace={workspace} projects={projects.data?.items ?? []} close={() => setCompose(false)} done={() => { setCompose(false); client.invalidateQueries({ queryKey: ['knowledge-artifacts', workspace.id] }); }} />)}
  </Page>;
}
