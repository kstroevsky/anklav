...
export function MemoryPage({ workspace }: { workspace: Workspace }) {
  ...
  const artifacts = useQuery<Array<{ id: string; title: string }>>({ queryKey: ['knowledge-artifacts', workspace.id], queryFn: () => api(`/workspaces/${workspace.id}/knowledge-artifacts`) });
  ...
  return <Page heading="Memory" subheading="Durable claims and decisions, with evidence, validity, and full supersession history." action={<button className="button primary" disabled={!projectId} onClick={() => setCompose(true)}>Propose {tab === 'claims' ? 'claim' : 'decision'}</button>}>
    ...
    {compose && (tab === 'claims' ? <ClaimComposer workspace={workspace} projectId={projectId} evidence={evidence.data ?? []} artifacts={artifacts.data ?? []} close={() => setCompose(false)} done={() => { setCompose(false); refresh(); }} /> : <DecisionComposer workspace={workspace} projectId={projectId} evidence={evidence.data ?? []} close={() => setCompose(false)} done={() => { setCompose(false); refresh(); }} />)}
  </Page>;
}
