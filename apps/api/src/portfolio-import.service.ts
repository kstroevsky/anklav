...
import {
  checklistItems, externalObjectMappings, externalSources, flows, githubConnections, githubProjectRepositories, githubRepositories, importBatches, importConflicts, importCreatedObjects, importVerifications,
  ...
  taskFlows, taskIdentifierAliases, tasks, workflowStates, workspaceMemberships, workspaces,
} from './db/schema';
...
export class PortfolioImportService {
  ...
  async plan(request: ImportRequest): Promise<Record<string, unknown>> {
    ...
    return {
      ...
      workspace: workspace ? { id: workspace.id, name: workspace.name, action: 'match' } : { name: 'Personal R&D', action: 'create' },
      ...summary, drift,
  ...
  async apply(request: ImportRequest, actor: AuthUser): Promise<Record<string, unknown>> {
    ...
    if (initialized.batch.status === 'completed') return { mode: 'apply', noOp: true, batch: initialized.batch, message: 'This exact bundle has already completed for the workspace.' };
  ...
  async verify(request: ImportRequest, actor: AuthUser, verificationReport: string): Promise<Record<string, unknown>> {
    ...
    const [batch] = await this.database.db.select().from(importBatches).where(and(eq(importBatches.workspaceId, workspace.id), eq(importBatches.bundleChecksum, bundle.bundleChecksum))).orderBy(desc(importBatches.createdAt)).limit(1);
    if (!batch) throw new NotFoundException('No import batch exists for this workspace and bundle.');
    const mappings = await this.database.db.select().from(externalObjectMappings).where(eq(externalObjectMappings.importBatchId, batch.id));
    const expectedKeys = new Set(bundle.records['source-mappings.ndjson']!.map((entry) => String(entry.sourceKey)));
    const mappingsBySource = new Map(mappings.map((mapping) => [mapping.sourceKey, mapping]));
    const missing = [...expectedKeys].filter((key) => !mappingsBySource.has(key));
    const drifts = mappings.filter((mapping) => mapping.status === 'drift');
    const report = {
      schemaVersion: '1.0', generatedAt: new Date().toISOString(), batch: { id: batch.id, status: batch.status, bundleVersion: bundle.manifest.schemaVersion, bundleChecksum: bundle.bundleChecksum },
      checksums: { valid: true, files: bundle.checksums.size }, expectedCounts: bundle.expectedCounts,
      outcomes: { sourceRecords: bundle.records['source-mappings.ndjson']!.length + bundle.records['knowledge-artifact-candidates.ndjson']!.length, bundleSourceMappings: bundle.records['source-mappings.ndjson']!.length, createdTargets: mappings.filter((entry) => entry.status === 'created').length, matchedTargets: mappings.filter((entry) => entry.status === 'matched').length, skippedTargets: mappings.filter((entry) => entry.status === 'skipped').length, reviewRequiredRecords: mappings.filter((entry) => entry.status === 'review_required').length, deferredRecords: mappings.filter((entry) => entry.status === 'deferred').length, failedRecords: mappings.filter((entry) => entry.status === 'failed').length },
      reconciliation: { everySourceMappingHasOutcome: missing.length === 0, missingSourceKeys: missing, driftSourceKeys: drifts.map((entry) => entry.sourceKey), originalIdentifiersPreserved: await this.originalIdentifiersPreserved(workspace.id, bundle), rawChatContentImported: false, credentialsImported: false },
      conflicts: bundle.conflicts, securityFindings: ['Verification output is outside the immutable bundle.', 'Bundle paths, symlinks, oversized files, and oversized NDJSON records are rejected before import.', 'No raw chat content exists in this bundle and session ingestion is not part of Phase 1.'],
      remainingHumanDecisions: this.preflight(bundle, request.overrides ?? {}, true).blocking,
    };
    ...
    await this.database.db.transaction(async (tx) => {
      ...
      await this.activity.append(tx, { workspaceId: workspace.id, subjectType: 'import_batch', subjectId: batch.id, action: 'verified', actor, after: { reportChecksum: checksum, everySourceMappingHasOutcome: missing.length === 0 } });
    });
  ...
  async rollback(request: ImportRequest, actor: AuthUser, guardedOverride = false): Promise<Record<string, unknown>> {
    ...
    await this.database.db.transaction(async (tx) => {
      ...
      await tx.update(externalObjectMappings).set({ status: 'rolled_back', updatedAt: new Date() }).where(eq(externalObjectMappings.importBatchId, batch.id));
  ...
  private overridesTemplate(bundle: MigrationBundle, supplied: ImportOverrides): ImportOverrides {
    ...
    return { sourceRepositoryVisibility: supplied.sourceRepositoryVisibility, projectControlTasks, milestoneClassifications };
  }

  private async initialize(bundle: MigrationBundle, selector: string, actor: AuthUser, overrides: ImportOverrides) {
    let workspace = await this.findWorkspace(selector);
    if (!workspace) {
      if (normalized(selector) !== normalized('Personal R&D')) throw new BadRequestException('A new migration workspace may only be created as Personal R&D.');
      const slug = slugify('Personal R&D');
      workspace = await this.database.db.transaction(async (tx) => {
        const [created] = await tx.insert(workspaces).values({ name: 'Personal R&D', slug, description: 'Imported Personal R&D portfolio.' }).returning();
        await tx.insert(workspaceMemberships).values({ workspaceId: created!.id, userId: actor.id, role: 'owner' });
        await tx.insert(workflowStates).values([
          ...DEFAULT_TASK_STATES.map(([name, semantic, color], position) => ({ workspaceId: created!.id, entityType: 'task' as const, name, color, taskSemantic: semantic, position, isInitial: position === 0 })),
          ...DEFAULT_FLOW_STATES.map(([name, semantic, color], position) => ({ workspaceId: created!.id, entityType: 'flow' as const, name, color, flowSemantic: semantic, position, isInitial: position === 0 })),
        ]);
        return created!;
      });
    }
    ...
    return this.database.db.transaction(async (tx) => {
      ...
      const [existing] = await tx.select().from(importBatches).where(and(eq(importBatches.externalSourceId, source!.id), eq(importBatches.bundleChecksum, bundle.bundleChecksum))).limit(1);
      if (existing) return { workspace: currentWorkspace, source: source!, batch: existing };
      const [batch] = await tx.insert(importBatches).values({ workspaceId: currentWorkspace.id, externalSourceId: source!.id, bundleVersion: bundle.manifest.schemaVersion, bundleChecksum: bundle.bundleChecksum, bundlePathHash: digest(bundle.root), status: 'applying', startedAt: new Date(), actorUserId: actor.id, overridesHash: digest(overrides) }).returning();
  ...
  private async findDrift(bundle: MigrationBundle, workspaceId: string): Promise<{ sourceKey: string; targetEntityType: string; targetEntityId: string | null }[]> {
    const existing = await this.database.db.select().from(externalObjectMappings).where(eq(externalObjectMappings.workspaceId, workspaceId));
  ...
  private async importMapping(tx: any, context: ImportContext, mapping: BundleRecord): Promise<ImportOutcome> {
    ...
    const [existing] = await tx.select().from(externalObjectMappings).where(and(eq(externalObjectMappings.workspaceId, context.workspaceId), eq(externalObjectMappings.sourceKey, sourceKey))).limit(1);
    if (existing) {
  ...
  private async importTask(tx: any, context: ImportContext, record: BundleRecord): Promise<ResolvedTarget> {
    ...
    const [created] = await tx.insert(tasks).values({ workspaceId: context.workspaceId, projectId, title: String(record.title), description, taskNumber: number, identifier: `${project!.issueKey}-${number}`, workflowStateId: state.id, priority: record.priority as any, humanReviewRequired: String((record.status as BundleRecord).semantic) === 'human_review', reviewStatus: String((record.status as BundleRecord).semantic) === 'human_review' ? 'pending' : 'not_required', verificationPerformed: section(description, 'Verification required'), completionEvidence: String(record.evidence ?? ''), remainingLimitations: section(description, 'Non-goals'), followUpWork: section(description, 'Follow-up work') }).returning();
  ...
  private async recordCandidateArtifacts(context: ImportContext, outcomes: Record<string, number>): Promise<void> {
    ...
    for (const candidate of candidates) await this.database.db.transaction(async (tx) => {
      ...
      if (!projectId) { outcomes.deferred = (outcomes.deferred ?? 0) + 1; return; }
      ...
      if (typeof repository !== 'string') { outcomes.deferred = (outcomes.deferred ?? 0) + 1; return; }
      const sourceKey = `project-control:artifact-candidate:${candidate.projectRef}:${candidate.path}`;
      const [mapped] = await tx.select().from(externalObjectMappings).where(and(eq(externalObjectMappings.workspaceId, context.workspaceId), eq(externalObjectMappings.sourceKey, sourceKey))).limit(1);
      if (mapped) return;
  ...
  private async originalIdentifiersPreserved(workspaceId: string, bundle: MigrationBundle): Promise<boolean> {
    const identifiers = bundle.records['tasks.ndjson']!.filter((task) => task.importDisposition === 'create_or_match').map((task) => String((source(task) ?? {}).identifier));
    if (!identifiers.length) return true;
    const rows = await this.database.db.select({ identifier: taskIdentifierAliases.identifier }).from(taskIdentifierAliases).where(and(eq(taskIdentifierAliases.workspaceId, workspaceId), sql`${taskIdentifierAliases.identifier} = ANY(${identifiers})`));
    return new Set(rows.map((row) => row.identifier)).size === identifiers.length;
  }
  ...
  private async softDeleteImportedObject(tx: any, type: string, id: string, actorId: string): Promise<void> {
    const fields = { deletedAt: new Date(), deletedByUserId: actorId, updatedAt: new Date() };
