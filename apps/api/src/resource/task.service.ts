...
import { activityEvents, checklistItems, comments, convergenceCriteria, flowAllowedProjects, flowRelations, flows, githubIssueLinks, githubPullRequests, githubRepositories, githubTaskPullRequests, labelAssignments, labels, projects, projectRepositories, repositories, projectTaskCounters, taskIdentifierAliases, taskFlows, taskRelations, tasks, users, workflowStates, workspaceMemberships } from '../db/schema';
...
export abstract class ResourceTaskService extends ResourceProjectFlowService {
  async listTasks(
    workspaceId: string,
    user: AuthUser,
  ...
  > {
    ...
    const rows = await (base as any)
      .where(and(...terms, filters.flowId ? eq(taskFlows.flowId, filters.flowId) : undefined, filters.labelId ? eq(labelAssignments.labelId, filters.labelId) : undefined, beforeUpdatedCursor(tasks.updatedAt, tasks.id, cursor)))
  ...
  async getTask(workspaceId: string, user: AuthUser, taskId: string) {
    ...
    const links = await this.database.db
      .select({ link: taskFlows, flow: flows })
      .from(taskFlows)
      .innerJoin(flows, eq(taskFlows.flowId, flows.id))
      .where(and(eq(taskFlows.taskId, task.id), isNull(flows.deletedAt)));
    ...
    const relations = await this.database.db
      .select()
      .from(taskRelations)
      .where(and(eq(taskRelations.workspaceId, workspaceId), or(eq(taskRelations.sourceTaskId, task.id), eq(taskRelations.targetTaskId, task.id))));
    ...
    const githubPullRequestLinks = await this.database.db
      .select({
        link: githubTaskPullRequests,
        pullRequest: githubPullRequests,
        repository: githubRepositories,
      })
      .from(githubTaskPullRequests)
      .innerJoin(githubPullRequests, eq(githubTaskPullRequests.pullRequestId, githubPullRequests.id))
      .innerJoin(githubRepositories, eq(githubPullRequests.repositoryId, githubRepositories.id))
      .where(eq(githubTaskPullRequests.taskId, task.id));
    const [targetRepository] = task.targetRepositoryId ? await this.database.db.select().from(repositories).where(eq(repositories.id, task.targetRepositoryId)).limit(1) : [];
    return {
      ...task,
      targetRepository: targetRepository ?? null,
      identifierAliases: aliases.map((entry) => entry.identifier),
      project,
      state,
      flows: links,
      checklists,
      relations,
      githubIssues: githubIssues.map(({ link, repository }) => ({
        ...link,
        repository,
      })),
      githubPullRequests: githubPullRequestLinks.map(({ link, pullRequest, repository }) => ({
        ...link,
        pullRequest: { ...pullRequest, repository },
      })),
      labels: await this.labelsFor('task', task.id),
      comments: await this.commentsFor('task', task.id),
      activity: await this.activityFor(workspaceId, task.id),
      transitionWarnings: await this.taskWarnings(workspaceId, task, state?.taskSemantic ?? 'inbox'),
    };
  }
  ...
  async createTask(workspaceId: string, user: AuthUser, input: z.infer<typeof taskInput>, idempotencyKey?: string) {
    ...
    const outcome = await this.database.db.transaction(async (tx) =>
      this.taskEvents.execute(tx, {
        workspaceId,
        idempotencyKey,
        command: { type: 'task.create', input },
        actor: user,
        operation: async () => {
          ...
          await this.membership(workspaceId, input.coordinatingMembershipId);
          if (input.targetRepositoryId) await this.requireProjectRepository(workspaceId, input.projectId, input.targetRepositoryId);
          ...
          const [task] = await tx
            ...
            .values({
              ...
              reviewStatus: values.humanReviewRequired ? 'pending' : 'not_required',
            })
            .returning();
  ...
  async updateTask(workspaceId: string, user: AuthUser, taskId: string, version: number, input: Partial<z.infer<typeof taskInput>>, idempotencyKey?: string) {
    ...
    const outcome = await this.database.db.transaction(async (tx) =>
      this.taskEvents.execute(tx, {
        workspaceId,
        idempotencyKey,
        command: { type: 'task.update', taskId, version, input },
        actor: user,
        operation: async () => {
          ...
          await this.membership(workspaceId, input.reviewerMembershipId);
          await this.membership(workspaceId, input.coordinatingMembershipId);
          const targetProjectId = input.projectId ?? before.projectId;
          const targetRepositoryId = input.targetRepositoryId === undefined ? before.targetRepositoryId : input.targetRepositoryId;
          if (targetRepositoryId) await this.requireProjectRepository(workspaceId, targetProjectId, targetRepositoryId);
          ...
          const reviewFields =
            input.humanReviewRequired === true && !before.humanReviewRequired
              ? {
          ...
          const [updated] = await tx
            ...
            .where(and(eq(tasks.id, before.id), eq(tasks.workspaceId, workspaceId), eq(tasks.version, version), isNull(tasks.deletedAt)))
            .returning();
          if (!updated)
          ...
          if (movedProject)
            await tx.insert(taskIdentifierAliases).values({
              workspaceId,
              taskId: before.id,
          ...
          await this.activityService.append(tx, {
            ...
            subjectType: 'task',
            subjectId: before.id,
            action: nextState ? 'updated_with_status_change' : 'updated',
            actor: user,
            before: selectChanged(before, values),
            after: selectChanged(updated!, values),
            metadata: transitionWarnings.length ? { transitionWarnings } : {},
          });
          const result = { ...updated!, transitionWarnings };
          return {
            aggregateId: updated!.id,
            aggregateVersion: updated!.version,
            eventType: nextState ? 'task.status_changed' : 'task.updated',
            state: updated!,
            result,
          };
        },
  ...
  private async requireProjectRepository(workspaceId: string, projectId: string, repositoryId: string) {
    ...
    const [link] = await this.database.db
      ...
      .where(and(eq(projectRepositories.projectId, projectId), eq(projectRepositories.repositoryId, repositoryId), eq(projects.workspaceId, workspaceId)))
      .limit(1);
    if (!link) throw new BadRequestException('The target repository must be linked to the task project.');
  }
  ...
  async transitionPreview(workspaceId: string, user: AuthUser, type: 'task' | 'flow', id: string, stateId: string) {
    ...
    if (type === 'task')
      return {
        warnings: await this.taskWarnings(workspaceId, await this.task(workspaceId, id), state.taskSemantic!),
      };
    return {
      warnings: await this.flowWarnings(workspaceId, await this.flow(workspaceId, id), state.flowSemantic!),
    };
  }
  ...
  async updateReview(workspaceId: string, user: AuthUser, taskId: string, version: number, input: z.infer<typeof reviewInput>, idempotencyKey?: string) {
    ...
    const outcome = await this.database.db.transaction(async (tx) =>
      this.taskEvents.execute(tx, {
        workspaceId,
        idempotencyKey,
        command: { type: 'task.review', taskId, version, input },
        actor: user,
        operation: async () => {
          const before = await this.task(workspaceId, taskId);
  ...
  private async validateFlowForTask(workspaceId: string, flowId: string, projectId: string) {
    ...
    if (flow.scope === 'selected_projects') {
      const [allowed] = await this.database.db
        .select()
        .from(flowAllowedProjects)
        .where(and(eq(flowAllowedProjects.flowId, flowId), eq(flowAllowedProjects.projectId, projectId)))
        .limit(1);
  ...
  protected override async ensureTaskLinkedToFlow(workspaceId: string, taskId: string, flowId: string) {
    ...
    const [link] = await this.database.db
      .select()
      .from(taskFlows)
      .where(and(eq(taskFlows.taskId, taskId), eq(taskFlows.flowId, flowId)))
      .limit(1);
