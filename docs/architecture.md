...
PostgreSQL full-text search uses the `simple` dictionary to preserve identifiers, paths, hashes, and error tokens. Lexical and vector candidates are generated independently behind the same hard workspace, project, task, sensitivity, and validity filters, then united for intent-aware reranking. This preserves exact-token recall and gives pgvector an indexable nearest-neighbor query instead of ordering the whole hybrid result by a combined SQL expression.

Embeddings are governed by revision-pinned profiles rather than a universal dimension constant. The first storage lane, `nomic-v2-768`, uses the native 768-dimensional output of `nomic-ai/nomic-embed-text-v2-moe`, its pinned model revision, cosine distance, a 512-token input contract, and distinct query/document prefixes. A profile declares its dimensions and storage lane; the service validates query vectors against that profile. Supporting a materially different dimension requires an explicit storage lane and index rather than silently padding, truncating, or mixing incompatible vectors.

Each source record is split into bounded, deterministic semantic units. The embedding input contains project/task/source identity, record and validity time, repository/Git metadata when known, classification, provider, authority, and sensitivity. Credential-like material is redacted before the derived embedding text is stored or sent to a provider, while canonical source content remains unchanged. Ordinary HTTP and MCP writers cannot upload vectors; only the trusted embedding-worker boundary may persist content-hash-guarded embeddings. Stale embeddings are excluded automatically when derived document content changes.

Refresh enqueues embedding work in the same PostgreSQL transaction that updates the derived documents. The separate worker claims bounded batches with `FOR UPDATE SKIP LOCKED`, reclaims expired leases, retries transient failures with capped exponential delay, dead-letters exhausted jobs, and marks changed documents as superseded. Immediately before each vector upsert it checks the document content hash again in the same transaction. Job status is inspectable through HTTP or MCP without exposing provider credentials.

The provider adapter uses an OpenAI-compatible `/embeddings` endpoint. Set `EMBEDDING_BASE_URL` to the endpoint's `/v1` base and set `EMBEDDING_MODEL_REVISION` to the exact revision declared by the active profile; a mismatch is rejected. `EMBEDDING_API_KEY` is optional for local endpoints. The API uses this adapter synchronously only when a search explicitly selects an embedding profile; otherwise lexical retrieval remains available without an embedding service. Start the durable document worker with `docker compose --profile embedding up -d --build`. The embedding profile is opt-in so the normal stack does not silently send derived project text to an unconfigured external service.

The 768-dimensional lane is intentional, not an assumption about every model: it preserves Nomic Embed v2's native representation and remains within pgvector's HNSW limits. Nomic's Matryoshka-compatible shorter output can be introduced later as a separate 256-dimensional lane only after the project evaluation set demonstrates an acceptable recall/storage tradeoff. Profile keys are revision-specific and should be replaced, not mutated, when a model revision or dimensional contract changes.
